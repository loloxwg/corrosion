...
/// - `node_interest`:interest 声明自身。否则鸡生蛋——没人「声明关心 node_interest」,
///   按 interest 过滤会把声明本身滤成零目标,传播只能靠 sync 兜底(慢,间隔拉长时可达分钟级)。
...
mod tests {
    ...
    fn ddl_log_broadcast_bypasses_interest_filter() {
        // corro_ddl_log(控制面/DDL 元数据)广播:即便 interest 已配置且没人"关心
        // corro_ddl_log",也必须全网可达——控制面元数据须比数据有更强传播保证,
        // 与 node_interest 的豁免同构。
        let a = cand(9000, Some(1));
        let b = cand(9001, Some(1));
        let candidates = vec![a, b];
        let mut routing = HashMap::new();
        ...
        assert_eq!(picked.len(), 2, "corro_ddl_log 广播不得被 interest 过滤,应全发");
    }
